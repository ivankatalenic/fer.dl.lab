...
def sample_gmm_2d(K, C, N):
	...
	Returns a tuple of samples and labels.
	'''
	indices = np.hstack([np.arange(C), np.random.choice(C, K - C)])
	samples = np.empty((K * N, 2), dtype=np.float32)
...
def eval_AP(Y_r):
	...
	Arguments:
		Y_r: List of correct labels sorted by their posterior probabilities.
	'''
	sum = np.sum(Y_r)
	total = Y_r.shape[0]
	AP = 0.0
...
def graph(X, Y, Y_, special=None):
	...
	Arguments:
		...
		special: Indices of samples that should be specially marked. Optional.
	'''
	marker_sizes = np.full((X.shape[0], 1), plt.rcParams['lines.markersize']**2, dtype=np.float64)
	if special is not None:
		marker_sizes[special] *= 2
	X = np.hstack([X, marker_sizes])
	
	class_cnt = np.max(Y_) + 1
	cmap = mpl.colormaps["gist_rainbow"]
	for class_id in range(class_cnt):
		color = cmap(class_id / (class_cnt - 1))

		correct = X[np.logical_and(Y_ == Y, Y_ == class_id)]
		plt.scatter(correct[:, 0], correct[:, 1], s=correct[:, 2], color=color, marker='o', edgecolor='black')

		incorrect = X[np.logical_and(Y_ != Y, Y_ == class_id)]
		plt.scatter(incorrect[:, 0], incorrect[:, 1], s=incorrect[:, 2], color=color, marker='s', edgecolor='black')
	plt.show()
...
def graph_surface(fun, rect, offset, width, height, decision_width=0.05):
	...
	samples = np.stack([xx.flatten(), yy.flatten()], axis=-1, dtype=np.float32)
	values = fun(samples)
